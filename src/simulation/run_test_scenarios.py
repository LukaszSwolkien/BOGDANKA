...
class TestRunner:
    ...
    def __init__(
        ...
        with profiles_path.open("r") as f:
            ...
            if profile_filter:
                self.profiles = [p for p in all_profiles if p["id"] in profile_filter or p["name"] in profile_filter]
                LOGGER.info(f"Filtered to {len(self.profiles)} profiles: {[p['name'] for p in self.profiles]}")
            else:
    ...
    def _calculate_total_estimated_time(self) -> float:
        ...
        try:
            with self.config_path.open("r") as f:
                ...
                default_acceleration = config_data.get("simulation", {}).get("acceleration", 1000.0)
        except Exception:
    ...
    def run_single_test(self, profile: dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            # Validate results
            validation_results = self._validate_results(actual_metrics, profile["expected_results"])
            ...
            if progress is not None and progress > 0:
                ...
                if progress_milestone > last_progress and progress_milestone >= 10:
                    ...
                    if progress > 5 and real_duration_s > 0:
                        ...
                        LOGGER.info(f"     [{bar}] {progress_milestone}% | ETA: ~{eta_str}")
                    else:
    ...
    def _collect_metrics(self, algo_app: Any) -> dict[str, Any]:
        ...
        total_op_time = sum(algo_app.algorithm_rn.get_heater_operating_time(h) for h in Heater)
    ...
    def _get_nested_value(self, data: dict[str, Any], key: str) -> Any:
        ...
        if key == "heater_balance_ratio" or key == "heater_balance_ratio_c2":
            ...
            operating_times = [h["operating_h"] for h in heaters.values() if h["operating_h"] > 0]
    ...
    def _validate_range(self, actual: Any, expected: dict[str, Any]) -> tuple[bool, str]:
        ...
        if isinstance(actual, list):
            if "min" in expected and "max" in expected:
                all_in_range = all(expected["min"] <= v <= expected["max"] for v in actual)
                if all_in_range:
                    return True, f"All values in range [{expected['min']}, {expected['max']}]"
                else:
                    out_of_range = [v for v in actual if v < expected["min"] or v > expected["max"]]
        ...
        if "target" in expected:
            ...
            messages.append(f"Target: {expected['target']}, Actual: {actual}, Diff: {diff:.2f}")
...
def generate_summary_report(results: list[TestResult]) -> str:
    ...
    for result in results:
        status_symbol = "✅" if result.passed else "❌" if result.status == "FAILED" else "⚠️"
